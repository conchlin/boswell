...
public class EventInstanceManager {
    ...
    private Map<Integer, Long> disconnectedCharacters = new HashMap<Integer, Long>();
    ...
    public void giveEventPlayersExp(int gain, int mapId) {
        ...
        if (mapId == -1) {
            ...
            for (MapleCharacter mc : players) {
                if (mc.getMapId() == mapId) mc.gainExp((int) (gain * mc.getExpRate()), true, true);
            }
        }
    }
    ...
    public void giveEventPlayersMeso(int gain, int mapId) {
        ...
        if (mapId == -1) {
            ...
            for (MapleCharacter mc : players) {
                if (mc.getMapId() == mapId) mc.gainMeso((int) (gain * mc.getMesoRate()));
            }
        }
    ...
    public void playerDisconnected(final MapleCharacter chr) {
        try {
            ...
            // So players can't just logout after dying and just get free warp back in
            if (chr.isAlive()) disconnectedCharacters.put(chr.getId(), System.currentTimeMillis());
        } catch (ScriptException | NoSuchMethodException ex) {
            ex.printStackTrace();
        }
    ...
    /**
     * The time when the player last disconnected. Used to calculate whether we should let them back
     * into the event or not
     * @param chr
     * @return time to compare to current time
     */
    public Long getDisconnectTime(MapleCharacter chr) {
        try {
            return disconnectedCharacters.get(chr.getId());
    ...
    /**
     * This should be called after a player rejoins so that they can't just deny and quickly rejoin or something.
     * @param chr MapleCharacter
     */
    public void removeDisconnect(MapleCharacter chr) {
        disconnectedCharacters.remove(chr.getId());
    }


    /**
     * 10 minutes
     * compare to getlogintime
     * @return
     */
    ...
    /**
     * Searches for player in disconnectedCharacters
     * if found and meets the rejoin requirements we warp them back in to event instance
     * @param chr
     * @return
     */
    public boolean allowJoin(MapleCharacter chr) {
        if (!disconnectedCharacters.containsKey(chr.getId())) {
            return false;
        } else {
            if (System.currentTimeMillis() - chr.getLoginTime() < getTimeoutWindow()) {
                removeDisconnect(chr);
                return true;
            }
        }
        chr.dropMessage(6, "It seems either you've passed the "+ getTimeoutWindow() / 60000 +" " +
                "minute timeout window to rejoin this event.");
        return false;
    }

    /**
     * Handler for PQ Tour drops
     */
    public final void givePQTourDrops(MapleCharacter player, String pqName){
        if(ENABLE_PQ_TOUR_FEATURE) {
            AbstractPlayerInteraction api = player.getAbstractPlayerInteraction();
            switch(pqName) {
                case KERNING_CITY_PQ:
                    api.gainItem(PQTour.handleRewards(KERNING_CITY_PQ), (short) 1);
                    break;
                case HENESYS_PQ:
                    api.gainItem(PQTour.handleRewards(HENESYS_PQ), (short) 1);
                    break;
                case LUDI_MAZE_PQ:
                    api.gainItem(PQTour.handleRewards(LUDI_MAZE_PQ), (short) 1);
                    break;
                case LUDIBIRUM_PQ:
                    api.gainItem(PQTour.handleRewards(LUDIBIRUM_PQ), (short) 1);
                    break;
                default:
                    break;
            }
