...
public class MapleMapItem extends AbstractMapleMapObject {
    ...
    public MapleMapItem(Item item, Point position, MapleMapObject dropper, MapleCharacter owner, MapleClient ownerClient, byte type, boolean playerDrop) {
        ...
        this.cheaterDrop = owner.isCheater();
    ...
    public final boolean canBePickedBy(MapleCharacter chr) {
        if (chr.getId() != character_ownerid) {
            if (this.isCheaterDrop() && !chr.isCheater() ||
            !this.isCheaterDrop() && chr.isCheater()) return false;
        }
    ...
    public void setCheaterDrop() {
        this.cheaterDrop = true;
    }

    public boolean isCheaterDrop() {
        return this.cheaterDrop;
    }
	...
	if (chr.needQuestItem(questid, getItemId())) {
	    this.lockItem();
            try {
                client.announce(DropPool.Packet.onDropEnterField(chr, this, null, getPosition(), (byte) 2));
            } finally {
                this.unlockItem();
            }
	}
	...
	client.announce(DropPool.Packet.onDropLeaveField(getObjectId(), 1, 0));
    }
}