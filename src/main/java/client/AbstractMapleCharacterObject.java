...
public abstract class AbstractMapleCharacterObject extends AbstractAnimatedMapleMapObject {
    ...
    private boolean assignStrDexIntLuk(Integer deltaStr, Integer deltaDex, Integer deltaInt, Integer deltaLuk) {
        ...
        try {
            ...
            if (newStr < 4 || newStr > ServerConstants.MAX_AP) {
                return false;
            }

            if (newDex < 4 || newDex > ServerConstants.MAX_AP) {
                return false;
            }

            if (newInt < 4 || newInt > ServerConstants.MAX_AP) {
                return false;
            }

            if (newLuk < 4 || newLuk > ServerConstants.MAX_AP) {
                return false;
            }
