 ...
 * @author Matze
 */
public final class StatChangeItemUseRequestHandler extends AbstractMaplePacketHandler {
    @Override
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (toUse != null && toUse.getQuantity() > 0 && toUse.getItemId() == itemId) {
            if (itemId == 2022178 || itemId == 2050004) {
                ...
                chr.cancelBuffStats(MapleBuffStat.DARKNESS);
                //chr.dispelDebuff(MapleDisease.DARKNESS);
                remove(c, slot);
                return;
            } else if (itemId == 2050002) {
                chr.cancelBuffStats(MapleBuffStat.WEAKEN);
                chr.cancelBuffStats(MapleBuffStat.SLOW);
                //chr.dispelDebuff(MapleDisease.WEAKEN);
                //chr.dispelDebuff(MapleDisease.SLOW);
                remove(c, slot);
                return;
            } else if (itemId == 2050003) {
                chr.cancelBuffStats(MapleBuffStat.SEAL);
                chr.cancelBuffStats(MapleBuffStat.CURSE);
