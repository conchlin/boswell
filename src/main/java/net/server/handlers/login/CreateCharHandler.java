...
public final class CreateCharHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        for (int item : items) {
            if (!isLegal(item)) {
                FilePrinter.printError(FilePrinter.EXPLOITS + name + ".txt", "Owner from account '"
                        + c.getAccountName() + "' tried to packet edit in char creation.");
        ...
        if (job == 0) { // Knights of Cygnus
            ...
            c.announce(CLogin.Packet.deleteCharResponse(0, CharDeleteResultType.CreationError.getState()));
            return;
        }
