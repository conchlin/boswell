    ...
    if (mode == -1) {
        ...
        if (cm.getMapId() == 100000200) {
            if (status == 0) {
                ...
                cm.sendSimple("#e#b<Party Quest: Primrose Hill>\r\n\r\n#b#k#n Hello, I'm Tory. Have you been to Primrose Hill? It's a beautiful hill where primroses bloom. I hear that a tiger named Growlie over at the Primrose hill is hungry. Won't You go with your party members and help Growlie?\r\n\r\n#L0#Go to Primrose Hill\r\n#L1#I would like to " + (cm.getPlayer().isRecvPartySearchInviteEnabled() ? "disable" : "enable") + " Party Search.\r\n#L2#Learn about Primrose Hill");
            } else if (status == 1) {
                if (selection == 0) {
                    if (cm.getParty() == null) {
        ...
        } else if (cm.getMapId() == 910010100) { // Talking to Tory before exit (No Bonus)
            var eim = cm.getEventInstance();
            if (status == 0) {
                cm.sendSimple("Thank you for aiding in the effort of feeding the Growlie. As a matter of fact, your team has already been rewarded for reaching this far. With this problem now solved, there is another issue happening right now, if you are interested check #bTommy#k there for the info. \r\n\r\n #L0#I would like to go to Henesys\r\n#L1#Turn in my extra Moon Bunny cakes");
            } else if (status == 1) {
                if (selection == 0) {
                    if (cm.getEventInstance().giveEventReward(cm.getPlayer())) {
                        eim.givePQTourDrops(cm.getPlayer(), "Henesys PQ");
                        cm.warp(100000200);
                    }
                } else  if (selection == 1) {
                    if (cm.hasItem(4001101, 15)) {
                        if (cm.canHold(1002798, 1)) {
                            cm.gainItem(4001101, -15);
                            cm.gainItem(1002798, 1);
                            cm.dispose();
                        } else {
                            cm.sendOk("Please free up some space in your EQP slot.");
                            cm.dispose();
                        }
                    } else {
                    cm.sendOk("It seems you dont have 15 Moon Bunny Cakes. Please check that first to get rewarded properly.");
                    cm.dispose();
                    } 
                }
            }
        } else if (cm.getMapId() == 910010400) { // Talking to Tory after exiting the Bonus Stage
            var eim = cm.getEventInstance();
            if (status == 0) {
