        ...
        if (status == 0) {
            if (player.getLevel() < exped.getMinLevel() && player.getLevel() > exped.getMaxLevel()) { //Don't fit requirement
                ...
                if (expedition.isInProgress()) {    // thanks Conrad for noticing exped leaders being able to still manage in-progress expeds
                    ...
                    eim.registerPlayer(player); // let the leader back in
                } else {
                    cm.sendSimple(list);
                    status = 2;
                }
                ...
                if (expedition.contains(player)) { //If you're registered, warp you in
                    var eim = em.getInstance(expedName + player.getClient().getChannel());
                    if(eim.allowJoin(player)) { // allow players back in
            ...
            if (selection == 1) {
                ...
                expedition = cm.getExpedition(exped);
                if(expedition != null) { // so we can only have one per channel?
