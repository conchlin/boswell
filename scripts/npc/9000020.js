...
var travelFrom = [777777777]; // 541000000 Boat Quay Town
...
function action(mode, type, selection) {
    ...
    if (travelStatus != -1) {
        if (status == 0) 
            cm.sendSimple("How's the traveling? Are you enjoying it?#b\r\n#L0#Yes, I'm done with traveling. " +
                "Can I go back to #m" + cm.getPlayer().peekSavedLocation("WORLDTOUR") +
                "#?\r\n#L1#No, I'd like to continue exploring this place.");
        else if (status == 1) {
            if (selection == 0) {
                cm.sendNext("Alright. I'll take you back to where you were before the visit to Japan. " +
                    "If you ever feel like traveling again down the road, please let me know!");
            } else if (selection == 1) {
        ...
        if (status == 0) {
            ...
            cm.sendNext("If you're tired of the monotonous daily life, how about getting out for a change? " +
                "There's nothing quite like soaking up a new culture, learning something new by the minute! " +
                "It's time for you to get out and travel. We, at the Maple Travel Agency, recommend you going " +
                "on a #bWorld Tour#k! Are you worried about the travel expenses? You shouldn't be! " +
                "We have carefully come up with a plan to let you travel for ONLY #b" +
                cm.numberWithCommas(travelFee[travelType]) + " mesos#k!");
        } else if (status == 1) {
            cm.sendSimple("We currently offer this place for your traveling pleasure: " +
                "\r\n#b" + travelPlace[travelType] +"#k. "
                + travelAgent[travelType] + " will be serving you there as the travel guide. " +
                "Rest assured, the number of destinations will increase over time. " +
                "Now, would you like to head over to the " + travelPlaceShort[travelType] + "?#b" +
                "\r\n#L0#Yes, take me to " + travelPlaceShort[travelType] +
                " (" + travelPlaceCountry[travelType] + ")");
        } else if (status == 2) {
            cm.sendNext("Would you like to travel to #b" + travelPlace[travelType] + "#k? " +
