...
def sel = npc.askMenu("Have you heard of the beach with a spectacular view of the ocean called #bFlorina Beach#k, " +
...
if (sel == 0) {
    if (user.getMeso() < 1500) {
        npc.say("I think you're lacking mesos. There are many ways to gather up some money, you know, like... selling your " +
    ...
    def ret = npc.askYesNo("So you have a #bVIP Ticket to Florina Beach#k? You can always head over to Florina Beach " +
    ...
    if (ret == 1) {
        if (user.haveItem(4031134)) {
            ...
            npc.say("Hmmm, so where exactly is your #bVIP Ticket to Florina\r\nBeach#k? Are you sure you have one? Please double-check.")
        }
    } else {
        npc.say("You must have some business to take care of here. You must be tired from all that traveling and hunting. " +
                "Go take some rest, and if you feel like changing your mind, then come talk to me.")
    }
} else if (sel == 2) {
    npc.sayNext("You must be curious about a #bVIP Ticket to Florina Beach#k. Haha, that's very understandable. " +
    ...
    npc.say("I came back without it, and it just feels awful not having it. Hopefully someone picked it up and put it " +
